  ...
  methods: {
    ...
    clickLevel(type, pid) {
      ...
      if (type == 1 && pid == -1) {
        ...
        // 新增机构，部门及岗位
        const treeId = this.modelGroupMechanismForm.tree_id + '-' + (this.$utils.length(this.modelGroupMechanismForm.children) > 0 ? this.modelGroupMechanismForm.children.length : '0');
        this.$set(addRowData, 'tree_id', treeId);
        this.$set(addRowData, 'treeId', treeId);
