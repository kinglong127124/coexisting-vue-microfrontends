...
<script>
  ...
  import { startLoading, endLoading } from '@/utilsCom/loading'
  import { getToken, getCookie } from '@/utilsCom/auth'
  import {
    getAttacheFileData,
    clearAttacheFile,
    ...
    methods: {
      ...
      getAttacheFileData(id) {
        if (this.$utils.length(id)) {
          getAttacheFileData({ tableId: id, type:(this.type=="documentUp") ? 4 : 1 }).then((res) => {
            endLoading()
            this.fileListArray = []
      ...
      async submitUpload(form) {
        ...
        const params = {}
        params.type =(this.type=="documentUp") ? 4 : 1   //documentUp表示资料管理和知识文档的文档上传时需要type==4

        params.medium = form.medium || ''
        params.dataTypeId = form.typeId || ''
        params.dataTypeName = form.typeName || ''

        params.tableId = form.id || ''
        params.tableCode = form.code || ''
        params.tableName = form.name || ''

        params.projectId = form.proId || ''
        params.projectCode = form.proCode || ''
        params.projectName = form.proName || ''
        ...
        params.sourceCode = resultDetail == 'Detail' ? name : this.$route.name || ''
        ...
        params.sourceName = resultTitle == '-详情' ? title : this.$route.meta.title || ''
