(function() {
  ...
  function createTable(options, e) {
    ...
    setGoWorkAndRestDay(options, datetable.count, options.restDay)
    $('#SY').val(Y)
    $('#SM').val(M)
  }
  ...
  function setconfigDayM(Y, M) {
    if (configDay['Y' + Y] && configDay['Y' + Y]['M' + M]) {
      configDayM = $.extend(true, {}, configDay['Y' + Y]['M' + M])
    } else {
      configDayM = {}
    }
  }
  ...
  function setWeek(options) {
    ...
    var week = 0
    while (alldays >= D) {
      week = parseInt((new Date(Y + '/' + M + '/' + D) - new Date(options.weekWalue)) / (3600 * 24 * 1000 * 7))
  ...
  // 设置上班休息日期
  function setGoWorkAndRestDay(options, count, dates = []) {
    var width = options.width
    var height = options.height
    var Y = options.date.getFullYear()
    var M = options.date.getMonth() + 1
    ...
    dates.forEach(item => {
      let date = new Date(`${Y}/${M}/${item.day}`)
      if (item.value === 1) {
        $('#days' + item.day).append('<div class="xbgj" xbgj="0" style="position: absolute;"><div class="rest" style="top:' +
          -parseInt((height - 16) / (count + 1) - 2) + 'px;background-color: #53a253">休</div></div>')
      } else {
        $('#days' + item.day).find('.xbgj').remove()
      }
  ...
  DateTable.prototype.monthDays = function() {
    if ((this.M == 2) && (this.Y % 400 == 0 || (this.Y % 100 != 0 && this.Y % 4 == 0))) {
      return 29
    } else {
      return calendar.solarMonth[this.M - 1]
    }
  }
  ...
  DateTable.prototype.leftHead = function() {
    ...
    for (var i = date.getFullYear() - 3; i < date.getFullYear() + 6; i++) this.html += '<option value="' + i + '">' + i + '</option>'
    this.html += '</select>'
    ...
    if (typeof option.initBtn === 'function') this.html += '<button type="button" class="init">初始化</button>'
    this.html += '</div>'
  }

  /* 获取左边区域的星期名称 */
  DateTable.prototype.leftWeek = function() {
    for (var i = 0; i < 7; i++) this.html += '<div class="week">' + calendar.Week[i] + '</div>'
  }
  ...
  DateTable.prototype.leftDay = function() {
    ...
    for (var i = 0; i < this.count * 7; i += 1) {
      if (i < this.W - 1 || i >= this.days + this.W - 1) {
        this.html += '<div class="days1"></div>'
      } else {
        day = i + 2 - this.W
        if (W == 6 || W == 7) {
          color = 'red'
  ...
  DateTable.prototype.lunarDay = function() {
    ...
    for (var i = 0; i < this.days; i += 1) {
      ...
      if (info.leapMonth[0] === lunar[2] && lunar[0] == 1) {
        ...
        flag = info.lunarMonth[lunar[2] - 1] === 0 ? 29 : 30
      }
      if (lunar[3] + 1 > flag) {
        lunar[3] = 1
  ...
  function rightArea(Y, M, D) {
    ...
    if (isclick) {
      html += '<input type="button" style="width:100%;" value="上  班" class="saveChange" />'
      html += '<input type="button" style="width:100%;" value="休  息" class="resetData" />'
    }
    $('.rightArea').empty()
  ...
  function getPushClick(Y, M) {
    $('.resetData').click(function() {
      if (typeof rest === 'function') {
        console.log(ClickDays)
        rest(Y, M, ClickDays, 1).then(res => {
          setGoWorkAndRestDay(option, tables.count, [{
            year: Y,
            month: M,
            day: ClickDays,
            value: 1
          ...
          $('.calendar').find('.init').click()
        })
      }
    })
    $('.saveChange').click(function() {
      var html = ''
      if (typeof goWork === 'function') {
        goWork(Y, M, ClickDays, 2).then(res => {
          setGoWorkAndRestDay(option, tables.count, [{
            year: Y,
            month: M,
            day: ClickDays,
          ...
          option.restDay.forEach(item => {
            if(item.year == Y && item.month == M && item.day == ClickDays) item.value = 2
          })
        }).catch(async () => {
          $('.calendar').find('.init').click()
        })
  ...
  function btnClick(options, e, count) {
    var Y = options.date.getFullYear()
    var M = options.date.getMonth() + 1
    var lowYear = new Date().getFullYear()
    // 选择框值改变触发
    ...
    $('.calendar').find('.init').on('click', async function() {
      if (typeof option.initBtn === 'function') {
        await option.initBtn(Y, M)
        reDraw(options, e)
      }
    })
    // 年份向前
  ...
  // 重新绘制日历表格
  async function reDraw(options, e) {
    var Y = $('#SY').val()
    var M = $('#SM').val()
    const q = await updateDate(Y, M)
    options.restDay = q
    $(e).empty()
  ...
  function setLayer(options, count) {
    ...
    $('.leftArea').css('width', parseInt(width * rate - 2) + 'px').css('height', height - 4 + 'px')
    $('.rightArea').css('width', parseInt(width * (1 - rate) - 2) + 'px').css('height', height - 4 + 'px')
