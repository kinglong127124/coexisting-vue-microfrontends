  ...
  <div class="login-container">
    <el-form
      ...
      <div v-show="isShowLogin" class="login-layout shade">
        ...
        <div class="qr-code-right" v-if="false">
...
<script>
  ...
  import { addClass, timeFix } from '@/utilsCom/index'
  import {
    getCode,
    qrcodeLoginCheck,
    abortRequest
  } from '@/api/user/login/index.js'
  import { setToken } from '@/utilsCom/auth'
      ...
      qrcodeLoginCheck() {
        qrcodeLoginCheck(this.loginId).then(res => {
          if (res.stats == 'waiting') {
            ...
            setToken(res.token)
            this.$router.push({ name: 'unitInfo', query: this.otherQuery })
