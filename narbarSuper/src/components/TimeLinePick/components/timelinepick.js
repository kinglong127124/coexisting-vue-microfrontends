...
import '@/assets/plugins/visualization/createjs';
import '@/assets/plugins/visualization/TweenMax';
...
export default class TimelinePick {
  ...
  create() {
    ...
    this.paddingX = 30;
    this.xScale = scaleLinear().domain([0, this.options.number]).range([0, this.stage.xp - (this.paddingX * 2)]);
  ...
  _createSlider(spanWidth, spanHeight) {
    ...
    slider.on('mouseover', function() {
      this.graphics.clear();
      var hoverColor = rgb(this.drawColor).brighter();
...
Date.prototype.add = function(interval, value) {
  ...
  switch (interval.toLowerCase()) {
    ...
    case 'month' :
      ...
      var newMonth = d.getMonth() + 1 + value;
      if (d.getMonth() > 12) {
        newMonth -= 12;
        newYear++;
      }
