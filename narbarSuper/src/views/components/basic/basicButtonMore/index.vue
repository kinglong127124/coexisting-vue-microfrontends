...
<template>
  <el-dropdown @command="command">
    <el-button icon="el-icon-menu" size="small" type="primary" plain plain>
      更多<i class="el-icon-arrow-down el-icon--right"></i>
    </el-button>
    <el-dropdown-menu slot="dropdown">
      <el-dropdown-item command="downloadTemplate">下载模板</el-dropdown-item>
      <el-dropdown-item divided command="importExcel">导入Excel</el-dropdown-item>
      <el-dropdown-item divided command="importExcelFront" v-if="false">前端导入Excel</el-dropdown-item>
      <el-dropdown-item command="exportExcel" v-if="false">导出Excel</el-dropdown-item>
    </el-dropdown-menu>
    ...
    <basic-dialog v-el-drag-dialog :visible.sync="visibleSyncImportExcelFront" title="导入Excel-选择">
      <upload-excel v-if="visibleSyncImportExcelFront" :on-successFront="successFront" :before-upload="beforeUploadFront"/>
      <div slot="footer" class="dialog-footer">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      clickOk() {
        let importData = this.$refs.refBasicUploadExcel.submitUpload({ ...this.pageInfo, id: '12', data: this.results })
