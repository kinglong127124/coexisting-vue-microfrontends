...
function uzStorage() {
  ...
  const os = (/os/gi).test(window.navigator.appVersion);
  const isAndroid = (/android/gi).test(window.navigator.appVersion);
...
u.isEmpty = function(source) {
  ...
  if (u.isObject(source)) return Object.keys(source) <= 0;
...
u.forEach = function(source, func) {
  ...
  let i = 0;
  for (const ikey in source) {
    const flag = func.apply(window, [(typeof (source) === 'object' ? ikey : i++), source[ikey]]);
    if (flag == false) break;
  }
};
...
u.isEqual = function(source1, source2, ignoreCase, ignoreSort) {
  ...
  if ((u.isArray(source1) && u.isArray(source2)) || (u.isObject(source1) && u.isObject(source2))) {
    ...
    if (source1 != source2) u.propEqual = false;
    return u.propEqual;
  }
};
...
u.string.split = function(source, separator, ignoreSpaceOrEmpty) {
  ...
  let items = source.split(separator);
...
u.string.getNext = function(code) {
  ...
  const int = parseInt(part2);
  const zero = (part2 + '.').split(int + '.')[0];
  const newPart2 = zero + (int + 1).toString();
...
u.array.indexOf = function(source, searchElement) {
  ...
  if (u.isObject(searchElement)) {
    ...
    for (const item of source) {
      index++;
      if (item.id && item.id == searchElement.id) {
        isHas = true;
        break;
      }
    }
...
u.array.group = function(array, fields) {
  ...
  u.forEach(array, function(i, item) {
    ...
    const index = u.array.indexOf(ret, itemGroup);
    if (index == -1) {
      itemGroup.group = [];
...
u.array.remove = function(array, values) {
  ...
  if (u.isEmpty(values)) {
    ...
    u.forEach(values, function(i, value) {
      const index = u.array.indexOf(array, value);
...
u.object.getValue = function(obj, propertyName, ignoreCase) {
  ...
  const pointIndex = propertyName.indexOf('.');
  if (pointIndex > -1) {
...
u.object.serialize = function(paramObj) {
  let ret = '', name, value, fullSubName, subName, subValue, innerObj, i;
...
u.object.remove = function(obj, keys) {
  ...
  if (!u.isEmpty(keys)) {
    ...
    u.forEach(obj, function(key, value) {
      const wrongful = (value === null || value === undefined || value === '');
      try {
...
u.base64.encrypt = function(input) {
  const str = CryptoJS.enc.Utf8.parse(input);
...
const userAgent = window.navigator.userAgent; // 获取浏览器的userAgent字符串
...
u.browser.callCamera = function() {
  if (u.browser.isIOS()) {
    const file = window.document.querySelectorAll('input[capture=camera]');
...
u.url.jumpToReferrer = function() {
  ...
  const url = search.split('referrer=')[1];
  window.location.href = url;
};
...
u.validate.positiveToFixed = function(input, digits) {
  digits = digits || 2;
  const reg = new RegExp('^[1-9][0-9]*$|^((0)|([1-9][0-9]*)).[0-9]{' + digits + '}$');
